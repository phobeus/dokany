...
DokanSendVolumeArrivalNotification(PUNICODE_STRING DeviceName) {
  ...
  targetName = DokanAllocNPagedZero(length);
...
DokanSendVolumeDeletePoints(__in PUNICODE_STRING MountPoint,
  ...
  point = DokanAllocNPagedZero(length);
  if (point == NULL) {
  ...
  deletedPoints = DokanAllocNPagedZero(olength);
...
InsertDeviceToDelete(PDOKAN_GLOBAL dokanGlobal, PDEVICE_OBJECT DiskDeviceObject,
  ...
  deviceEntry = DokanAllocNPagedZero(sizeof(DEVICE_ENTRY));
  if (deviceEntry == NULL) {
  ...
  if (SessionId != -1) {
    deviceEntry->MountPoint.Buffer = DokanAllocNPaged(MountPoint->MaximumLength);
...
InsertMountEntry(PDOKAN_GLOBAL dokanGlobal, PDOKAN_CONTROL DokanControl,
  ...
  mountEntry = DokanAllocNPagedZero(sizeof(MOUNT_ENTRY));
  if (mountEntry == NULL) {
...
DokanAllocateUnicodeString(__in PCWSTR String) {
  ...
  PWSTR buffer;
  ULONG length;
  unicode = DokanAllocNPaged(sizeof(UNICODE_STRING));
  if (unicode == NULL) {
    return NULL;
  }

  length = (ULONG)(wcslen(String) + 1) * sizeof(WCHAR);
  buffer = DokanAllocNPaged(length);
  if (buffer == NULL) {
    ExFreePool(unicode);
    return NULL;
  }
...
DokanCreateDiskDevice(__in PDRIVER_OBJECT DriverObject, __in ULONG MountId,
  ...
  __try {
    DokanLogInfo(&logger,
                 ...
                 MountPoint, MountId);
    diskDeviceNameBuf = DokanAllocNPagedZero(MAXIMUM_FILENAME_LENGTH * sizeof(WCHAR));
    symbolicLinkNameBuf =
        DokanAllocNPagedZero(MAXIMUM_FILENAME_LENGTH * sizeof(WCHAR));
    mountPointBuf = DokanAllocNPagedZero(MAXIMUM_FILENAME_LENGTH * sizeof(WCHAR));
    dokanControl = DokanAllocNPagedZero(sizeof(DOKAN_CONTROL));
