...
// Creates a buffer from DokanAllocNPaged() containing
...
// if DokanAllocNPaged() fails, then it returns STATUS_INSUFFICIENT_RESOURCES
...
NTSTATUS DokanGetParentDir(__in const WCHAR *fileName, __out WCHAR **parentDir,
  ...
  *parentDir = (WCHAR *)DokanAllocNPagedZero((len + 1) * sizeof(WCHAR));
...
--*/
{
  ...
  __try {
    ...
    if (relatedFileObject != NULL // Get RelatedFileObject filename.
      ...
      if (relatedCcb->Fcb) {
        ...
        if (relatedFcb->FileName.Length > 0 &&
          ...
          relatedFileName = DokanAllocNPaged(sizeof(UNICODE_STRING));
          ...
          relatedFileName->Buffer =
              DokanAllocNPaged(relatedFcb->FileName.MaximumLength);
    ...
    fileName = DokanAllocNPagedZero(fileNameLength + sizeof(WCHAR));
    if (fileName == NULL) {
